...
$BlobObjectInitializeDisk = @{
    FileLocation = "./28_InitializeDisk.ps1"
    ObjectName   = "28_InitializeDisk.ps1"
}

# Upload files to the specified storage container with the specified blob name
$blobIISConfig = Set-AzStorageBlobContent -Context $StorageAccount.Context -Container $ContainerName -File $BlobObjectIISConfig.FileLocation -Blob $BlobObjectIISConfig.ObjectName
$blobInitializeDisk = Set-AzStorageBlobContent -Context $StorageAccount.Context -Container $ContainerName -File $BlobObjectInitializeDisk.FileLocation -Blob $BlobObjectInitializeDisk.ObjectName
...
$blobURLs = @($blobIISConfig.ICloudBlob.Uri.AbsoluteUri, $blobInitializeDisk.ICloudBlob.Uri.AbsoluteUri)
# $blobInitializeDiskURL = @($blobInitializeDisk.ICloudBlob.Uri.AbsoluteUri)
...
$settings = @{
    "fileUris" = $blobURLs  # Separate fileUris for each script
}
...
$protectedSettings = @{
    ...
    "commandToExecute"   = "powershell -ExecutionPolicy Unrestricted -File 26_IIS_Config.ps1 && powershell -ExecutionPolicy Unrestricted -File 28_InitializeDisk.ps1"
}

# Set the Azure VM extension with specified configurations and settings
Set-AzVMExtension -ResourceGroupName $resourceGroup -Location $location -VMName $vmName -Name "IISExtensionDiskExtension" -Publisher "Microsoft.Compute" -ExtensionType "CustomScriptExtension" -TypeHandlerVersion "1.10" `
