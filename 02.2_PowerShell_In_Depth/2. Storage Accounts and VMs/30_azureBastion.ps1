...
# Create a subnet configuration for Bastion
$bastionSubnetName = "AzureBastionSubnet"
$bastionSubnetAddressSpace = "10.0.1.0/24"
$bastionSubnet = New-AzVirtualNetworkSubnetConfig -Name $bastionSubnetName -AddressPrefix $bastionSubnetAddressSpace
...
New-AzVirtualNetwork -Name $networkName -ResourceGroupName $resourceGroup `
    -Location $location -AddressPrefix $AddressPrefix -Subnet $subnet, $bastionSubnet
...
# Create a new public IP address with specified settings: name, resource group, location, SKU, and allocation method
$publicIPAddress = New-AzPublicIpAddress -Name $publicIPaddressName -ResourceGroupName $resourceGroup -Location $location -Sku "Standard" -AllocationMethod "Static"

# Define the name for the bastion host
$bastionName = "app-bastion"

# Create a new Azure Bastion service within the specified resource group, using the previously created public IP address and virtual network
New-AzBastion -ResourceGroupName $resourceGroup -Name $bastionName -PublicIpAddress $publicIPAddress -VirtualNetworkId $VirtualNetwork.Id
